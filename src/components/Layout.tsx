  ...
  return (
    <div className="bg-[#101415] text-[#e0e3e5] min-h-screen font-body select-none">
        ...
        <div className="flex justify-between items-center px-6 h-16 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/" className="flex items-center gap-2">
              <span className="text-2xl font-black text-primary tracking-tighter uppercase font-display bg-gradient-to-r from-[#ffffff] to-[#00e0ff] bg-clip-text text-transparent">
                Speedesk
              </span>
            </Link>
          </div>
          ...
          {/* Global Search Bar */}
          <div className="hidden md:flex flex-1 max-w-md mx-8 items-center bg-[#191c1e] rounded-xl px-4 py-1.5 border border-white/5 focus-within:border-primary/40 transition-all">
            <span className="material-symbols-outlined text-on-surface-variant text-sm select-none">search</span>
            <input 
              type="text" 
              placeholder="Buscar apresentações, slides e kits..." 
              value={searchQuery}
          ...
          <div className="flex items-center gap-4 md:gap-5">
            ...
            <div 
              className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-[#191c1e] border border-white/5 hover:border-primary/20 transition-all select-none h-9"
            ...
            >
              ...
              >
                <span className="material-symbols-outlined text-[17px] text-primary align-middle group-hover:scale-105 transition-transform" style={{fontVariationSettings: "'FILL' 1"}}>account_balance_wallet</span>
                <span className="text-xs font-mono font-bold text-[#e1e3e3] hidden sm:inline-block select-none">
                  {isBalanceVisible 
            ...
            >
              <div className="w-9 h-9 rounded-full bg-[#272a2c] overflow-hidden border border-white/10 group-hover:border-primary/50 transition-colors flex items-center justify-center">
                {profile.avatar ? (
                  <img 
                    src={profile.avatar} 
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-[#191c1e] text-[#8b9293]">
                    <svg className="w-5 h-5 text-[#8b9293]/80" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="hidden lg:flex flex-col text-left">
                ...
                <span className={`text-[9px] font-mono uppercase tracking-wider leading-none font-bold ${
                  profile.role === 'Admin' 
                    ? 'text-primary' 
      ...
      <div className="flex pt-16 min-h-[calc(100vh-4rem)]">
        ...
        {/* Sidebar Left */}
        <aside 
          className={`fixed md:sticky top-16 left-0 h-[calc(100vh-4rem)] z-40 bg-[#101415] border-r border-white/5 p-4 w-64 transition-all duration-300 ${
            isSidebarOpen ? 'translate-x-0 opacity-100' : '-translate-x-full md:w-0 md:p-0 md:border-r-0 md:opacity-0 pointer-events-none'
          }`}
          id="main-sidebar"
        >
          {/* Navigation Links */}
                ...
                return (
                  ...
                  >
                    <span className={`material-symbols-outlined text-xl transition-transform duration-200 ${
                      isActive ? 'text-primary scale-105' : 'text-[#bac9cd]/70 group-hover:text-primary-container'
                    ...
                    {/* Hover Glow Pill */}
                    {isActive && (
                      <span className="absolute right-3 w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(0,224,255,0.8)]" />
                    )}
                  </Link>
                );
              })}
          ...
          {isSidebarOpen && (
            <Link 
              ...
              className="mx-1 p-4 rounded-xl bg-gradient-to-br from-[#1d2022] to-[#101415] border border-white/5 hover:border-primary/30 shadow-md block transition-all hover:scale-101 cursor-pointer group"
            >
              <div className="flex justify-between items-start mb-1">
                <p className="text-[10px] font-mono text-[#baf2ff] uppercase tracking-widest font-bold">Plano Speedesk</p>
          ...
          {isSidebarOpen && (
            <div className="mt-4 mx-1">
              ...
              >
                <span className="material-symbols-outlined text-[18px] font-bold">cloud_upload</span>
          ...
          {isSidebarOpen && (
            <div className="mt-4 mx-1">
              <button 
                onClick={onLogout}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-all font-black uppercase text-xs tracking-wider cursor-pointer font-mono"
          ...
          {isSidebarOpen && (
            <div className="absolute bottom-6 left-4 right-4 text-center">
              <div className="text-[10px] font-mono text-[#bac9cd]/30 uppercase tracking-widest">
                SPEEDESK PLATFORM v1.4
              </div>
              <div className="text-[8px] font-mono text-primary/30 mt-1 uppercase tracking-wider">
                Aetheric Flux Space
              </div>
            </div>
          )}
        ...
        {/* Content Area */}
        <main className="flex-1 px-4 py-8 md:px-10 max-w-7xl mx-auto w-full transition-all duration-300">
          <Outlet />
        </main>
      </div>
    </div>
